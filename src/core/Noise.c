...
Rasteron_Image* noiseImgOp_lattice(ImageSize size, ColorLattice lattice){
...
Rasteron_Image* noiseImgOp_warp(ImageSize size, const ColorLatticeTable* latticeTable){
	...
	for(unsigned l = 0; l < latticeTable->latticeCount; l++)
		*(noiseImages + l) = noiseImgOp_lattice(size, latticeTable->lattices[l]);
