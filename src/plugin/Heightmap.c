...
Rasteron_Heightmap* createHeightmap(const Rasteron_Image* refImage){
	...
	for (unsigned p = 0; p < heightmap->width * heightmap->height; p++)
		if(p >= (heightmap->width * heightmap->height) - heightmap->width - heightmap->width) *(heightmap->data + p) = computeHeight(*(refImage->data + p), heightmap->minBound, heightmap->maxBound);
