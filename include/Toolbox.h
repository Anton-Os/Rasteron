...
void switchRasterRG(uint32_t* raster, unsigned int pCount); // switch red and green bits
...
uint32_t itrpolate(uint32_t color1, uint32_t color2, double iVal); // interpolate between hi and low colors
uint32_t blend(uint32_t color1, uint32_t color2, double bVal); // blends values between 1st and second colors
